...
! function(global, factory) {
    var DI = factory();
    !window['DI'] && (window['DI'] = DI, window.define = DI.define, window.cmd = true);
}(window, function() {
    /*
     * module manager
     */
    var ModuleMgr = (function() {
        ...
        DI = {
            ...
            use     : _use,
            base    : '',
            setPath : function(path) {DI.base = path;},
            cache   : {},
            release : 0
        },
        loadingScripts  = [],
    ...
    proto.loadDeps = function() {
        if (DI.release || this.deps.length == 0) {
    ...
    proto.loaded = function() {
        ...
        if (this.isDepsLoaded()) {
            ...
            DI.cache[this.name] = this.exports
